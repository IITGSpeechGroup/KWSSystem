...
def main(fn):
   ...
   #PATH=SYSTEM_PATH+'model_epoch_50.model'
   PATH=SYSTEM_PATH+'Mizo_C20_model_epoch_50.model'
   #model.load_state_dict(torch.load(PATH))
   model.load_state_dict(torch.load(PATH, map_location="cpu"))

   model.eval()

   keyword = np.genfromtxt(SYSTEM_PATH+'keyWordList_mizo',dtype='str')
